...
output-file: git-github-ssh-basic-workflow.html
---
...
2. **Clone the Repository to Your Local Computer:**
   ...
   - Use the `git clone` command followed by the SSH URL of your newly created GitHub repository.
...
* Step 2: Click the green "Code" button, and choose **SSH** in the pop-up.
  You set up your SSH key earlier in this lecture,
  so this is the option to use.
  The URL you get looks like `git@github.com:github_username/repository_name.git`
  rather than starting with `https://`.
...
* Step 5: Type git clone followed by the URL and press <kbd>Enter</kbd>:

  ```bash
  git clone git@github.com:github_username/repository_name.git
  ```

  You may also see an HTTPS URL offered,
  which looks like `https://github.com/github_username/repository_name.git`.
  That works too,
  but it will ask you to authenticate every time you push.
  With your SSH key set up, the `git@` URL will not.
...
* Step 1: Start by navigating to the GitHub you want the repository on,
  and ensure you are logged in.
  MDS uses two:
  <https://github.com> is the public GitHub,
  and <https://github.ubc.ca> is UBC's private GitHub Enterprise.
  They are separate sites with separate accounts.

  :::{.callout-important}
  Check which one an assignment asks for before you create anything.
  Coursework that has to be publicly visible,
  such as a personal website published with GitHub Pages,
  belongs on <https://github.com>.
  A repository created on the wrong site cannot simply be moved,
...
and then interactively choose which parts you want by typing either <kbd>n</kbd> (no)
or <kbd>y</kbd> (yes) followed by pressing <kbd>Enter</kbd> for each chunk of code.
...
you should write the command `git commit` and press <kbd>Enter</kbd>.
...
Once you have committed, your changes are saved locally on your laptop.
...
git knows that you want to push your changes to that location by default,
and you can do so by typing `git push` without specifying a URL.

You will also see the longer form written out in full:

```bash
git push origin main
```

This says the same thing explicitly.
`origin` is the default name git gives the remote you cloned from,
and `main` is the branch you are on.
Being explicit is a good habit,
and some assignments will ask you to type it this way
so that it is obvious what is going where.
