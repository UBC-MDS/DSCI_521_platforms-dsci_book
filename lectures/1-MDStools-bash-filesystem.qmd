...
::::
::::{.activity-container}
You have already learned how to use some commands using JupyterLab IDE. Do you find it easier to use the command line or JupyterLab user interface?

Why might one be better than the other? And in what scenarios?

A. The command line is better for scripting and automation because it allows for quick execution of commands and integration into automated workflows.

B. JupyterLab is better for interactive data exploration and visualization because it provides a rich user interface with features like the command palette and inline plotting.

C. The command line is better for beginners because it has a simpler interface.

D. JupyterLab is better for large-scale data processing because it handles more data efficiently.

E. Both command line and JupyterLab are equally easy and effective in all scenarios.
::::
:::
...
Would they be possible to use with the command `ls` instead of `cd`?
...
::::
::::{.activity-container}
You are asked to run the following commands in your home directory and explore the content of the folder example_521 using your favorite file manager (Finder for Mac, File Explorer for Windows, and any file manager in Linux). Can you draw a diagram with the content of that folder?

What command should you run in the shell to find the folder find_me?
...
:::{.activity}
::::{.activity-header}
## Lecture 1 Activity 6
::::
::::{.activity-container}
My home directory is: `/Users/Varada/`
The output of running `pwd` in the command line is:
`/Users/Varada/canadian_languages`

What is the problem if I run the command:
`cd Users/Varada`.
How can I solve it?
