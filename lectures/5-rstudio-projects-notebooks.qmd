...
You have the [following report]() and you have realized that there was an error in one of the calculations you did to generate it. This means that several results are going to be modified.
...
Introducing RStudio! An Interactive Development Environment (IDE),
...
* Way 4. *Set the working directory using the console*: Type `setwd("PATH")` in the console, replacing "PATH" with the path to your desired directory. Use this method with care, as explained in Jenny's [article on workflow vs. script](https://www.tidyverse.org/articles/2017/12/workflow-vs-script/).
...
::::
::::{.activity-container}
Let's go back to the document of the first activity. How can we make changes to one of the calculation?

A. Yes, by using dynamic documents to automatically update the results

B. No, manually rerunning the scripts is the only way

C. Yes, by creating a new report from scratch each time

D. No, it's best to keep the original report unchanged
...
Question: Check the documentation of [Pandoc Markdown](https://pandoc.org/MANUAL.html#pandocs-markdown) and [GitHub Flavour Markdown](https://github.github.com/gfm/#what-is-github-flavored-markdown-). Which of the following is true?
    
A. Pandoc Markdown and GitHub Flavour Markdown render the same way.

B. GitHub Flavour Markdown supports emojis, while Pandoc Markdown does not.
    
C. Pandoc Markdown cannot be converted to HTML.
    
D. Both render exactly the same across all platforms.
...
::::
::::{.activity-container}
Is an R Notebook the same as an .Rmd document?
...
::::
::::{.activity-container}
Clone a folder and try to render the .Rmd file. What happens? Try to use here() for the filepath.

    a) The file renders without any issues.
    b) The file fails to render due to incorrect file paths.
    c) The file renders but with missing plots.
    d) The file cannot be opened in RStudio.
::::
:::
