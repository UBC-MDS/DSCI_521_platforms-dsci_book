...
output-file: quarto-github-pages.html
---
...
We can use our arrow keys to select the project type we want, and hit <kbd>Enter</kbd> to create the skeleton for that project.
...
The contents of the `_quarto.yml` (in this example) file tells quarto that we are going to be
creating a website.
...
Just note the indentation, white space, and dashes `-` are all really important and imply different things.
...
### Finding what you need in the Quarto documentation {#quarto-docs}

The Quarto documentation is already an excellent resource.
We don't want to reproduce all of it in this textbook.
Also, reading a tool's own documentation is a skill in itself,
and one you will use for every tool you meet from here on,
so this is a good place to start practising it.

The [Quarto Guide](https://quarto.org/docs/guide/) is the entry point.
Think of it as the table of contents for everything Quarto can do.

#### Site-wide settings, in `_quarto.yml`

- **Navigation bar**, the links across the top of every page,
  set with `website: navbar:` ---
  [Website Navigation](https://quarto.org/docs/websites/website-navigation.html)
- **Listing pages**, a page that indexes other pages for you,
  set with `listing:` ---
  [Website Listings](https://quarto.org/docs/websites/website-listings.html)
- **Blogs**, the whole pattern of a `posts/` folder plus a listing page ---
  [Creating a Blog](https://quarto.org/docs/websites/website-blog.html)
- **Appearance**, set with `theme:` ---
  [HTML Theming](https://quarto.org/docs/output-formats/html-themes.html)
- **Publishing**, including the `output-dir: docs` approach we use below ---
  [Publishing to GitHub Pages](https://quarto.org/docs/publishing/github-pages.html)

#### Page content, in your `.qmd` files

- **Markdown syntax**: headings, lists, links, bold, tables ---
  [Markdown Basics](https://quarto.org/docs/authoring/markdown-basics.html)
- **Images and figures**, including captions and sizing,
  written as `![A caption.](path/to/image.jpg){width=250px}` ---
  [Figures](https://quarto.org/docs/authoring/figures.html)
- **About pages**, a built-in layout that puts a photo beside your text,
  set with `about:` in the document's YAML header ---
  [About Pages](https://quarto.org/docs/websites/website-about.html)

#### Running code in your pages

You do not need these for a site that is only text and images,
but you will once you write posts that contain an actual analysis.

- **Running R** ---
  [Using R](https://quarto.org/docs/computations/r.html)
- **Running Python** ---
  [Using Python](https://quarto.org/docs/computations/python.html)
- **Controlling what a cell shows**,
  for example hiding the code but keeping the plot,
  written as `#| echo: false` inside the cell ---
  [Execution Options](https://quarto.org/docs/computations/execution-options.html)

:::{.callout-tip}
## Reading documentation is part of the job

When you look one of these up,
notice how the page is put together:
a short explanation,
then a small complete example you can copy,
then a reference table of every option.
Most good documentation is arranged this way.
Skimming for the example first,
and dropping to the reference table only when the example is not enough,
will get you an answer faster than reading top to bottom.
:::
...
We do this with a `.nojekyll` file,
which we will set up in [Turning off Jekyll with `.nojekyll`](#nojekyll).
...
#### Turning off Jekyll with `.nojekyll` {#nojekyll}

There is one more file to create before the site will publish reliably.

As we mentioned earlier,
GitHub Pages runs [Jekyll](https://jekyllrb.com/) over your repository before it serves it.
Jekyll is a site generator, similar to some Quarto features,
and it expects to be handed a Jekyll source project to build (similar to the Quarto qmd files),
not the finished `html` that `quarto render` produces.

Jekyll skips over files and folders whose names begin with an underscore (`_`) or a dot (`.`),
which is a problem when everything in `docs` is part of the site we want served.

We turn Jekyll off by putting an empty file named `.nojekyll`
in the folder GitHub publishes from.
This is what the Quarto documentation recommends as well,
see [Publishing to GitHub Pages: Render to `docs`](https://quarto.org/docs/publishing/github-pages.html#render-to-docs).

Create the file at the top level of your repository:
...
Quarto copies `.nojekyll` into your `output-dir` every time you render,
so after your next `quarto render` you will have both
`.nojekyll` and `docs/.nojekyll`.

You can confirm it arrived with `ls -a`,
the same `-a` flag we used to look for a `.git` folder:
...
:::{.callout-warning}
## Create `.nojekyll` at the top level, not inside `docs`

`quarto render` empties `output-dir` before it fills it again.
A `.nojekyll` that you create directly inside `docs` is deleted by the next render,
and the site quietly goes back to being processed by Jekyll.

Creating it at the top level avoids this,
because Quarto copies it back in on every render.
:::

What GitHub Pages actually looks for is `.nojekyll` in the folder it publishes,
so the file that matters is `docs/.nojekyll`.
Check that it is on github.com after you push,
...
:::{.callout-tip}
## If your website renders incorrectly

If your local quarto html files look coorect, but github pages seems to render something different.
Then check that the rendering location (e.g., `docs/` or the project root `/`) has the `.nojekyll` file.
If it doesn't then you may need to add a step to put it in there.
:::
